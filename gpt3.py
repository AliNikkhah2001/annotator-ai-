...
fig.update_layout(height=1000, hovermode="x unified", dragmode="pan", title=f"{tf_now} Chart",margin=dict(l=10,r=10,t=10,b=10))
...
# full dataset candlesticks so panning never blanks
fig.add_trace(
    go.Candlestick(
        x=df["Date"],
        open=df["Open"],
...
for i, cfg in enumerate(SESSION_INFO.values()):
    for day in pd.to_datetime(dff["Date"].dt.date.unique()):
        ...
        fig.add_shape(
            type="rect", x0=sx, x1=ex, xref="x", y0=y0_p, y1=y1_p, yref="paper", fillcolor=cfg["color"], line=dict(width=0), layer="above"
        )
...
for iso, m in st.session_state.annotations.get(tf_now, {}).items():
    ...
    fig.add_trace(
        go.Scatter(x=[dt], y=[m["y"]], mode="markers", marker=dict(size=3, color=LABEL_OPTIONS[m["label"]][1]), showlegend=False)
    )
